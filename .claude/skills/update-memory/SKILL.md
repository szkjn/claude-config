---
name: update-memory
description: Update project memory after a session (create or update .claude/memory/*.md files)
---

Review the current conversation and update project memory.

Memory is an **architectural knowledge base**, not a session log or changelog. Its purpose is to capture things that can't be derived from reading the code or git history.

## Steps

1. Read `CLAUDE.md` to see the memory table and existing file list
2. Read all files in `.claude/memory/` to understand what's already documented
3. Review the conversation and recent git diff to understand what happened
4. Update memory:
   - **Existing file needs update**: edit the relevant `.md` in `.claude/memory/`
   - **New feature/topic**: create a new `.md` in `.claude/memory/` and add a row to the Memory table in `CLAUDE.md`
   - **General pattern discovered**: add to `CLAUDE.md` directly (not memory)
5. **Cleanup pass**: check all existing memory files for stale content - references to removed features, deprecated approaches, or details that are now irrelevant. Trim or delete as needed.
...
- Deprecated or removed features (delete these from memory)

## Rules

- Be concise. Every token costs money and context space.
- No fluff, no redundancy with CLAUDE.md.
- Memory files are feature-specific and evolving. CLAUDE.md is general and stable.
